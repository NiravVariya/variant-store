...
import { Box, Button, CloseButton, Flex, Input, Modal, ModalBody, ModalCloseButton, ModalContent, ModalFooter, ModalHeader, Select, Spinner, Stack, Table, TableContainer, Tbody, Td, Text, Th, Thead, Tr, useDisclosure } from "@chakra-ui/react";
...
function EditVarients(props: any) {
    const { fetchVariants, fetchVariantsCombination } = props;
    ...
    useEffect(() => {
        if (fetchVariants) {
            setVarientList(fetchVariants)
        }
        if (fetchVariantsCombination) {
            setVariantCombination(fetchVariantsCombination)
        }
    }, [fetchVariants, fetchVariantsCombination])
    ...
    const handleAddImages = async (index: number, value: any) => {
        let copyCombination = structuredClone(variantCombination);
