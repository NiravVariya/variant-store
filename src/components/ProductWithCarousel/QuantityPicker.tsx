...
export const QuantityPicker = (props: QuantityPickerProps) => {
  const { min = 1, max, rootProps, handleValue, ...rest } = props

  const [value, setValue] = useControllableState(rest)
  const handleDecrement = () => {setValue(value === min ? value : value - 1); handleValue(value - 1)}
  const handleIncrement = () => { setValue(value === max ? value : value + 1); handleValue(value + 1) }
